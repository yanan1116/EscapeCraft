...
class RequiredAsset:
    ...
    def sample(self, other):
        candidates = []
        for (self_asset, self_num), (other_asset, other_num) in zip(self.assets.items(), other.assets.items()):
            if self_num > other_num:
                candidates.append(self_asset)
...
class HouseGenerator:
    def __init__(
        ...
        max_floor_objects: Optional[int] = None,
    ) -> None:
        ...
        self.required_assets = RequiredAsset(required_assets) if required_assets else None
        self.max_floor_objects = max_floor_objects if max_floor_objects is not None else 10
        ...
        if priority_asset_types:
            for asset_type in priority_asset_types:
                ...
                if can_spawn_standalone:
                    # print(asset_candidates.index)
                    if priority_asset:
                        assets_with_type = asset_candidates[
                            asset_candidates.index.isin([priority_asset])
                        ]
                    else:
                        assets_with_type = asset_candidates[
    ...
    def generate(
        ...
        max_floor_objects = self.max_floor_objects
        ...
        for room in self.rooms.values():
            ...
            if self.required_assets:
                _generated = {asset: 0 for asset in self.required_assets.assets}
                generated_assets = RequiredAsset(_generated)
            for i in range(max_floor_objects):
                ...
                # print(spawnable_assets)
                if self.required_assets and self.required_assets.sample(generated_assets):
                    asset = self.required_assets.sample(generated_assets)
                    ...
                    try:
                        self.required_assets.assets[asset] += 1
                    except:
                        pass
                else:
                    asset = self.sample_and_add_floor_asset(
        ...
        small_object_instances, __placer = add_small_objects(
            object_instances,
            odb,
            self.rooms,
